...
# --- ABA DRE ---
    ...
    if lancamentos and contas:
        ...
        st.table(dre_data)
        # PDF
        if st.download_button("Baixar PDF", data=gerar_relatorio_consolidado(st.session_state.user.email, 0, lucro, dre_data, df[['nome_conta', 'valor']].head(10)), file_name="relatorio.pdf"):
            st.success("Download!")
