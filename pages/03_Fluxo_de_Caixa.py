...
if lancamentos and contas:
    ...
    # Padronização para comparação (Remove espaços e ignora maiúsculas)
    df['status_limpo'] = df['status_financeiro'].astype(str).str.strip().str.capitalize()

    # 3. FILTRO RÍGIDO: Apenas Entrada e Saída
    # Aqui filtramos o DataFrame para descartar qualquer outra coisa como "Transação Interna"
    df = df[df['status_limpo'].isin(['Entrada', 'Saída'])].copy()

    # 4. Cálculo do Valor Efetivo
    # Entrada = Valor positivo | Saída = Valor negativo
    def calcular_efetivo(row):
        if row['status_limpo'] == 'Entrada':
            return row['valor']
        else: # Já filtramos apenas Entrada/Saída, logo se não é Entrada, é Saída
