...
if st.session_state.view_mode == "Plano de Contas":
    ...
    # Lista de grupos com natureza devedora
    ...
    # Agrupa por Conta para mostrar todos os razonetes
    contas_unicas = df_filtered['Conta'].unique()
    cols = st.columns(2)
    
    for i, conta in enumerate(contas_unicas):
        # Filtra lançamentos da conta
        ...
        # Cálculo de Saldo baseado na Natureza
        ...
        with cols[i % 2]:
            ...
            st.markdown(f'<div class="t-header">{conta.upper()} ({grupo_conta})</div>', unsafe_allow_html=True)
