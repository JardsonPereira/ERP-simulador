...
# --- Configuração ---
# O set_page_config deve ser sempre o primeiro comando de layout
st.set_page_config(layout="wide", page_title="Contabilidade")
...
if st.session_state.view_mode == "Razonetes":
    ...
    for grupo in grupos:
        ...
        for conta in df_grupo['Conta'].unique():
            ...
            st.markdown(f"<div style='background-color: {bg_color}; color: white; padding: 5px; text-align: center; font-weight: bold; border-radius: 5px; margin-top: 5px;'>{label_saldo}</div>", unsafe_allow_html=True)
