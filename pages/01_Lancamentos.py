...
if user_id:
    ...
    if res.data:
        # CONVERSÃO SEGURA PARA PANDAS
        df = pd.DataFrame(res.data)
        id_para_nome = {v: k for k, v in dicionario_contas.items()}
        
        # Adiciona colunas para exibição
        df["Excluir"] = False
        ...
        df_edit = df[colunas_exibicao]
        
        # DATA EDITOR COM CONFIGURAÇÃO DE COLUNA SEGURA
        edit = st.data_editor(
            df_edit, 
        ...
        if st.button("💾 Salvar Edições"):
            try:
                # Compara o DataFrame editado com o original via ID
                for i in range(len(edit)):
                    if not edit.iloc[i]["Excluir"]: # Apenas se não marcado para excluir
                        row = edit.iloc[i]
        ...
        if st.button("🗑️ Excluir Selecionados"):
            ids_para_deletar = df.loc[edit["Excluir"], "id"].tolist()
            if ids_para_deletar:
